//
//  Extensions.swift
//  TinderApp
//
//  Created by adam janusewski on 3/10/22.
//

import SwiftUI

//  Just pasted this code below.  Very common extension in SwiftUI development.  If conditional is true, transform it to something else.
//  In this case, will transform the images to red if cli

extension View {
    @ViewBuilder
    func `if`<Transform: View>(
        _ condition: Bool,
        transform: (Self) -> Transform
    ) -> some View {
        if condition {
            transform(self)
        } else {
            self
        }
    }
}

extension Color {
    static let textPrimary = Color.gray.opacity(0.9)
    static let textSecondary = Color.gray.opacity(0.6)
    static let textTitle = Color.black.opacity(0.8)
    static let defaultBackground = Color(.systemGray6).opacity(0.35)
    static let textFieldBackground = Color(.systemGray6)
}


// Closes Keyboard
extension View {
    func endEditing(_ force: Bool) {
        UIApplication.shared.windows.forEach { $0.endEditing(force)}
    }
}

// Takes in some content, returns some view, returning hidden navigationBar
struct HideNavigationView: ViewModifier {
    func body(content: Content) -> some View {
        content
            .navigationTitle("")
            .navigationBarHidden(true)
    }
}
